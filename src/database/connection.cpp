...
namespace database {
	...
	{
		INFO("DB::CONN", "New database connection to {}", settings.to_string());
	}
	...
	void connection::prepare_statement(const std::string& name, const std::string& query) {
		...
		if (prepared_statements_.find(name) != prepared_statements_.end()) {
			ERR("DB::CONN", "Prepared statement \"{}\" already exists", name);
			abort();
		}
		...
		try {
			...
			ERR("DB::CONN", "Failed to prepare statement {{ \"{}\": \"{}\" }}, {}", name, query, e.what());
			abort();
		}
	}
