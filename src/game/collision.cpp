...
namespace detail {
	...
	inline Vec sign(const Vec& value)
	{
...
double AABB::left() const { return center.x - half.x; }
double AABB::right() const { return center.x + half.x; }
double AABB::top() const { return center.y - half.y; }
double AABB::bottom() const { return center.y + half.y; }
...
std::optional<Result> cast(const Ray& ray, const AABB& object) {
	...
	const auto nearTimeX = (object.center.x - (sign.x * object.half.x) - ray.start.x) * scale.x;
	const auto nearTimeY = (object.center.y - (sign.y * object.half.y) - ray.start.y) * scale.y;
	const auto farTimeX = (object.center.x + (sign.x * object.half.x) - ray.start.x) * scale.x;
	const auto farTimeY = (object.center.y + (sign.y * object.half.y) - ray.start.y) * scale.y;
	...
	auto normal = nearTimeX > nearTimeY ? Vec{ -sign.x, 0 } : Vec{ 0, -sign.y };
	auto contact = Vec{
		ray.start.x + (ray.delta.x * toi),
		ray.start.y + (ray.delta.y * toi)
	};
	auto velocity = Vec{
...
std::optional<Hit> sweep(const Swept<AABB>& swept, const std::vector<AABB>& objects) {
	// inflate each object
