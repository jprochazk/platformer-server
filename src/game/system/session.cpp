...
namespace game {
    namespace system {
        ...
        void session::on_error(uint32_t id, std::string_view what, beast::error_code error) {
            ...
            ERR("SOCKET", "ID {} -> [{}] ERROR: {}", id, what, error.message());
        }

        void session::handle_connections() {
            ...
            for (size_t i = 0; i < num_connections; i++) {
                ...
                INFO("SESSION", "ID {} joined", sessions[i].id);
                // TODO: move this somewhere
                registry.emplace<component::session>(entities[i], std::move(sessions[i]));
                registry.emplace<component::input>(entities[i], component::input{ false,false,false });
                registry.emplace<component::body>(entities[i], component::body{
                ...
                registry.emplace<component::collider>(entities[i], component::collider{ 
                    collision::AABB{ collision::Vec{32,32}, collision::Vec{32,32} }
                });
        ...
        void session::handle_disconnections() {
            ...
            registry.view<component::session>().each(
                ...
                if (auto it = sdisconnections.find(s.id); it != sdisconnections.end()) {
                    INFO("SESSION", "Session ID {} left", s.id);
