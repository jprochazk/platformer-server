...
movement::update()
{
    ...
    registry.view<input, body>().each([&](const entt::entity&, const input& input, body& body) {
        if (body.mtype == body::mtype::WALK) {
            // jump state machine
            if (body.jump == body::jump_state::NONE) {
                body.vel.x = 0.0;
                if (input.left)
                    body.vel.x += -body.speed;
                if (input.right)
                    body.vel.x += body.speed;
            ...
            if (input.right)
                body.vel.x += body.speed;
            if (input.up)
                body.vel.y -= body.speed;
            if (input.down)
                body.vel.y += body.speed;
        }
    });

    registry.view<body>().each([&](const entt::entity&, body& body) {
        body.lastPos = body.pos;
        body.pos += body.vel;
    });

    registry.view<body, collider>().each([&](const entt::entity& e, body& body, collider& col) {
        ...
        if (min.x < 0)
            col.box.center.x = col.box.half.x;
        if (min.y < 0)
            col.box.center.y = col.box.half.y;
        if (max.x > bounds.x)
            col.box.center.x = bounds.x - col.box.half.x;
        if (max.y > bounds.y) {
