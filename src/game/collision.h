...
/*
template<typename T = void>
bool test(Swept<AABB> swept, std::vector<Collider<AABB, T>>& colliders,
Callback<T> callback) {
        // defaults
        auto nearestResult = Result{
Vec{},Vec{},Vec{},std::numeric_limits<double>::max() }; auto nearestObject =
colliders.end();
        ...
        if (swept.velocity.x == 0 && swept.velocity.y == 0) {
                ...
                for (auto it = colliders.begin(); it != colliders.end(); it++) {
                        auto result = cast(ray, AABB{ it->shape.center,
it->shape.half + swept.shape.half }); if (result && result->time <
nearestResult.time) { nearestResult = *result; nearestObject = it;
                        }
                }
        }
