...
std::unordered_set<entt::entity> const&
zone::get_entities() const
{
    return entities;
}
std::vector<zone::object>&
zone::get_objects()
{
    return objects;
}
std::vector<zone::object> const&
zone::get_objects() const
{
    return objects;
}
collision::Vec
zone::get_bounds() const
{
    return bounds;
}
...
void
map::insert(entt::entity e, zone::id id)
{
...
void
map::transfer(entt::entity e, zone::id to)
{
    if (auto it = zone_ids.find(e); e != zone_ids.end()) {
        auto& [_, id] = *it;
        zones[id].get_entities().erase(e);
...
zone const&
map::get_zone(entt::entity entity) const
{
