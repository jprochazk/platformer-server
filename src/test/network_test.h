...
class client : public std::enable_shared_from_this<client>
{
  public:
    ...
    ~client() { client_handler_->on_close(); }
  ...
  private:
    ...
    {
        if (ec)
            return client_handler_->on_error("on_resolve", ec);
    ...
    {
        if (ec)
            return client_handler_->on_error("on_handshake", ec);
    ...
    {
        if (ec)
            return client_handler_->on_error("on_read", ec);
    ...
    {
        if (ec)
            return client_handler_->on_error("on_write", ec);
...
class test_server_handler : public network::socket_handler
{
  public:
    ...
    void set_on_close(std::function<void(uint32_t)> fn) { on_close_fn = fn; }
...
class test_client_handler : public network::test::client_handler
{
  public:
    ...
    void set_on_open(std::function<void()> fn) { on_open_fn = fn; }
    ...
    void set_on_close(std::function<void()> fn) { on_close_fn = fn; }
...
class network_test : public ::testing::Test
{
  public:
    ...
    bool get_status() { return success.load(std::memory_order_acquire); }
...
dispatch_test_task(net::io_context& ioc, std::function<void()> fn)
{
