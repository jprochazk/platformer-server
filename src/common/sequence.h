...
class sequence
{
  public:
    static_assert(std::is_fundamental<T>::value,
                  "Sequence number type must be fundamental!");
    using number_type = T;

    sequence(number_type start = static_cast<number_type>(0),
    ...
    number_type get()
    {
        ...
        current_.store(current_.load(std::memory_order_acquire) + increment_,
