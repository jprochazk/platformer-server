...
socket::open()
{
    ...
    ws_.async_accept(
      beast::bind_front_handler(&socket::on_accept, shared_from_this()));
}
...
socket::send(std::vector<uint8_t> data)
{
    net::post(ws_.get_executor(),
              beast::bind_front_handler(
                &socket::on_send, shared_from_this(), std::move(data)));
}
...
socket::on_accept(beast::error_code ec)
{
    ...
    ws_.async_read(
      fbuffer_,
      beast::bind_front_handler(&socket::on_read, shared_from_this()));
}
...
socket::on_read(beast::error_code ec, std::size_t read_bytes)
{
    ...
    if (read_bytes > 1) {
        ...
        boost::asio::buffer_copy(boost::asio::buffer(buf.data(), read_bytes),
                                 fbuffer_.data());
