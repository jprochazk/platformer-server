...
void
handle_input(game::world& world, const entt::entity& entity, const json& packet)
{
    ...
    if (!registry.has<component::input>(entity))
        return;
...
void
handle_toggle_mtype(game::world& world, const entt::entity& entity, const json&)
{
    using namespace game;

    auto& registry = world.get_registry();

    if (!registry.has<component::body>(entity))
        return;
...
const handler&
default_handler_table::get(uint16_t value)
{
