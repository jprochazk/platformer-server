...
inline void fail(std::string_view what, beast::error_code ec) {
    ...
    WARN("SOCKET_LISTENER", "{}: {}", what, ec.message());
}
...
struct default_socket_handler : public network::socket_handler {
    ...
    virtual void on_error(uint32_t id, std::string_view what, beast::error_code error) override {
        ...
        // just log the error
        ERR("SOCKET_LISTENER", "Socket ID {} -> error: {}, {}", id, what, error.message());
    }
};
